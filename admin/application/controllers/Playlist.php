...
class playlist extends CI_Controller {
    ...
    public function add()
    {
        ...
        if(($this->session->userdata('admid'))) {
            ...
            $data['message']='';
            $this->load->view('add_playlist_view',$data);
    ...
    public function addaction()
    {
        ...
        if(($this->session->userdata('admid'))) {
            ...
            $this->form_validation->set_rules('playlisturl', 'Play list url', 'required|trim');
            ...
            {
                $explodearr=explode('=',$this->input->post('playlisturl'));
                $playlistid=end($explodearr);
                // $playlist= $this->playlist_model->get_playlist($id);

                $data2 = array(
                    'title' => $this->input->post('title'),
                    'play_list_url' => $this->input->post('playlisturl'),
                    'play_list_id'=>$playlistid
                );

                if($this->playlist_model->insert_playlist($data2))
                {
    ...
    public function playlist_list()
    {
        ...
        $config['base_url'] = base_url()."index.php/playlist/playlist_list/";
        $config['total_rows'] = $this->playlist_model->count_playlist();
        $config['per_page'] = 50;
        ...
        $data["allplaylists"] = $this->playlist_model->pagination_select_playlist($config["per_page"], $page);
        ...
        $this->load->view('includes/header_view');
        $this->load->view('includes/navigation_view');
        $this->load->view('list_playlist_view',$data);
        $this->load->view('includes/footer_view');
    }
    ...
    public function delete()
    {
        ...
        if($this->playlist_model->delete_playlist($id))
        {
    ...
    public function edit($id)
    {
        ...
        if(($this->session->userdata('admid'))) {
            ...
            $data['message']='';
            $this->load->view('edit_playlist_view',$data);
            $this->load->view('includes/footer_view');
        }
        else
        {
    ...
    public function editaction($id)
    {
        ...
        if(($this->session->userdata('admid'))) {
            ...
            $this->load->view('includes/header_view');
            $this->load->view('includes/navigation_view');
            $data['channels']=$this->channel_model->select_all_channel();
            ...
            if ($this->form_validation->run()== FALSE)
            {
               ...
               // $playlist= $this->playlist_model->get_playlist($id);
                ...
                $this->playlist_model->update_playlist($data2, $id);
