...
$apps = DB::table("tbl_apps")->select("tbl_apps.*",
  DB::raw("IF(tbl_apps.publish_date IS  NULL , '',tbl_apps.publish_date ) as pdate"),
       DB::raw("(SELECT count(*) FROM tbl_visits WHERE tbl_visits.app_id = tbl_apps.id) as appvisit"),
      DB::raw("(SELECT count( distinct tbl_visits.ip_address) FROM tbl_visits WHERE tbl_visits.app_id = tbl_apps.id ) as  uniquevisit"))->where('user_id', $userObj->id)->paginate(15)->appends(['sort' => 'votes'])->toArray();
   $message = " Result Here..";
return Response::json(array('error' => false,'message' => $message,'totalviews'=>$totalviews,'totaluviews'=>$totaluniqueviews->uviews,'apps'=> $apps),200);
//return $apps;
}
else
{
