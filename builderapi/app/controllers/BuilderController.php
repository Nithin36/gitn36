...
else
{
    ...
    $socialimg_path_org = Input::get("id");
    $app_obj = new Apps();
    $app = $app_obj->find(Input::get("appid"));
    if($app)
    {
        ...
        foreach ($html->find('img') as $item) {
            ...
            if($t!=$socialimg_path_org)
            {
                $replace=$replace.'<li data-src="'.$t.'"><img src="'.$t.'"></li>';
        ...
        $updatedcontent= str_replace("$newcontent3","$replace",$contents);
...
return Response::json(array('error' => false,'message' => $message),200);
    ...
    public function deleteGalleryPhotoOld()
    {
...
//                        //print_r($app);
                ...
                if($data['hemail']==1)
                {
                     if($qp->find('.ftp-hidden')->attr('value')=="floatmenu")
                   {
                       $qp->find('.ftb-email')->attr('style','display:block');
                   }
                   else
                   {
                ...
                if($data['hcnumber']==1)
                {
                       if($qp->find('.ftp-hidden')->attr('value')=="floatmenu")
                   {
                        $qp->find('.ftb-contact-number')->attr('style','display:block');
                   }
                   else
                   {
...
//                        //print_r($app);
                    ...
                    if(trim($data['videourl'])!="")
                    {
                             if($qp->find('.ftp-hidden')->attr('value')=="floatmenu")
                   {
                             $qp->find('.ftp-video-title')->attr('style','display:block'); 
                         }
                         else
                         {
